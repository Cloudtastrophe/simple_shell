 ...
 * @ptr: Pointer to the list of arguments of the print_error function.
 * @buffer: Buffer to store the printable character.
...
int print_int_format(va_list ptr, char *buffer, int *buffer_index)
{
	...
	int x = 0;
	unsigned int tmp = (i < 0) ? -i : i;  /* Use tmp for positive value */
